...
public class AppRest {
	...
	@GetMapping("/users")
	public List<Utilisateur> f() {
		return dao_u.findAll();
	}
	@GetMapping("/user/{id}")
	public Optional<Utilisateur> f4(@PathVariable("id") int id) {
		return dao_u.findById(id);
	}
	
	@GetMapping("/messagesrecus/{id}")
	public List<Message> f1(@PathVariable("id") int id) {
		return dao_m.getAllMessagesReceiveByIdUser(id);
